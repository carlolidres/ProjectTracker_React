-- Only create/link endorsement from Support save when a real endorsement number is provided.
-- Matches Project Entry behavior: blank/N/A opens Endorsement Tracker as New (?new=1)
-- instead of inserting a stub that can collide on endorsement_tracker_active_number_idx.
...
BEGIN
  ...
  v_endorsement_number := nullif(trim(p_activity->>'endorsement_number'), '');
  IF p_create_endorsement
     AND v_endorsement_number IS NOT NULL
     AND lower(v_endorsement_number) <> 'n/a'
  THEN
    endorsement_result := public.ensure_endorsement_from_source(
      'non_process_support_activity',
      v_activity_id,
      jsonb_build_object(
        'endorsement_number', v_endorsement_number,
